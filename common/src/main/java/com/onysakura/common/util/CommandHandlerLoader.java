package com.onysakura.common.util;
...
public class CommandHandlerLoader {
    ...
    private static ICommandHandler loadCommandHandler(Enum<Commands> command) throws CommandHandlerLoaderException {
        ...
        if (resourceAsStream==null){
            LOGGER.error(ExceptionMessageConstants.COMMANDHANDLERS_FILE_NOT_FOUND);
            throw  new CommandHandlerLoaderException(ExceptionMessageConstants.COMMANDHANDLERS_FILE_NOT_FOUND);
        }
        ...
        try (final BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(resourceAsStream, "utf-8"))) {
            ...
            while(!StringUtils.isEmpty(s=bufferedReader.readLine())){
                ...
                if (split.length!=2){
                    LOGGER.error(ExceptionMessageConstants.COMMANDHANDLERS_FILE_CONFIG_ERROR);
                    throw new CommandHandlerLoaderException(ExceptionMessageConstants.COMMANDHANDLERS_FILE_CONFIG_ERROR);
                }
                ...
                if (command.name().equals(key)) {
                    ...
                    if (inter == null) {
                        throw new CommandHandlerLoaderException(ExceptionMessageConstants.COMMAND_HANDLER_ERROR);
                    }
